...
const SOLUTIONS_SHOWN = [
  { title: 'Маршрутизация входящих заявок', short: 'Заявки с сайта, Авито и мессенджеров попадают в один поток. Система определяет продукт, регион и срочность, отсеивает спам и отдаёт заявку нужному менеджеру', audience: 'Тем, у кого заявки идут из нескольких каналов и теряются в общей куче', status: 'delivered', statusLabel: 'Внедрено у клиента', statusHint: 'Работает в проде, есть кейс' },
  { title: 'Автоматический монтаж коротких видео', short: 'Система находит в длинной записи законченные фрагменты, режет паузы, переводит в вертикальный формат, держит спикера в кадре и добавляет субтитры', audience: 'Тем, у кого копятся вебинары, подкасты и выступления без нарезки', status: 'prototype', statusLabel: 'Рабочий прототип', statusHint: 'Собрано и проверено, но не у клиента' },
  { title: 'Автозаполнение CRM по звонкам', short: 'После разговора система расшифровывает запись, достаёт договорённости, суммы и сроки, заполняет карточку сделки и ставит следующую задачу', audience: 'Отделам продаж, где карточки заполняются по памяти и с опозданием', status: 'concept', statusLabel: 'Можем собрать', statusHint: 'Компетенции есть, готового решения нет' },
];
  ...
  return (
    <>
      ...
      <Section>
        ...
        <p style={{ maxWidth: 'var(--container-prose)', margin: '0 0 var(--space-10)', fontSize: 'var(--text-lg)', color: 'var(--text-secondary)', lineHeight: 'var(--leading-relaxed)' }}>
          Готовые сценарии, которые собираются под ваш процесс. У каждого честно указано, внедряли ли мы его у клиента или пока только можем собрать.
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 'var(--space-6)' }}>
          {SOLUTIONS_SHOWN.map((s, i) => (
            <Reveal key={s.title} delay={i * 60}><SolutionCard {...s} audienceLabel="Кому" /></Reveal>
          ))}
        </div>
        <p style={{ marginTop: 'var(--space-6)', fontSize: 'var(--text-sm)', color: 'var(--text-muted)', display: 'inline-flex', alignItems: 'center', gap: 'var(--space-2)' }}>
          <Icon name="lightbulb" size={16} />В проде карточек тринадцать — здесь показаны три статуса подряд.
        </p>
      </Section>
